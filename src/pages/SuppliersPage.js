...
export default function SuppliersPage() {
  ...
  // --- DÉMOS DE DONNÉES (statique) ---
  const seed = [
    {
      id: 1,
      nom: 'PharmaSup Madagascar',
      adresse: 'Lot II H 45, Antananarivo',
      telephone: '+261 34 12 345 67',
      email: 'contact@pharmasup.mg',
      contact: 'Rasolo Andry',
      paiement: '30 jours fin de mois',
      delai: 5,
      statut: 'Actif',
      createdAt: '2024-11-02',
      notes: 'Fournisseur principal génériques',
    },
    {
      id: 2,
      nom: 'MediDistrib Tana',
      adresse: 'Rue Ravelojaona, Ankadifotsy',
      telephone: '+261 32 88 999 10',
      email: 'sales@medidistrib.mg',
      contact: 'Ralaivo Nomena',
      paiement: 'Comptant',
      delai: 2,
      statut: 'Inactif',
      createdAt: '2025-01-15',
      notes: 'En renégociation des tarifs',
    },
    {
      id: 3,
      nom: 'Océan Pharma',
      adresse: 'Zone Franche, Tamatave',
      telephone: '+261 20 53 212 10',
      email: 'hello@oceanpharma.mg',
      contact: 'Harinirina Soa',
      paiement: '45 jours',
      delai: 7,
      statut: 'Actif',
      createdAt: '2025-03-08',
      notes: 'Spécialiste dispositifs médicaux',
    },
  ];
  ...
  const actionsBody = (row) => (
    <div style={{ display: 'flex', gap: 6 }}>
      ...
      <Button
        icon={row.statut === 'Actif' ? 'pi pi-ban' : 'pi pi-check'}
        className="p-button-text p-button-sm"
        onClick={() => toggleStatut(row)}
        tooltip={row.statut === 'Actif' ? 'Désactiver' : 'Activer'}
      />
  ...
  const addressBody = (row) => (
    <span>
      {row.adresse}
      <br />
      <small style={{ color: '#6b7280' }}>{row.telephone}</small>
    </span>
  );
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#eef1f2' }}>
      ...
      {/* Bandeau module (style client lourd) */}
      <div
        style={{
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <i className="pi pi-truck" style={{ fontSize: 18 }} />
          <strong>Stock ▸ Achat ▸ Gestion des fournisseurs</strong>
        </div>
      ...
      {/* Dialog Ajout / Édition */}
      <Dialog
        header={form.id ? 'Modifier le fournisseur' : 'Nouveau fournisseur'}
        visible={showForm}
      ...
      >
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          ...
          <span className="p-float-label">
            <InputText id="delai" value={form.delai} onChange={(e) => setForm({ ...form, delai: e.target.value })} style={{ width: '100%' }} />
      ...
      {/* Dialog Historique des commandes (mock) */}
      <Dialog
        header={`Historique — ${historyFor?.nom ?? 'Fournisseur'}`}
        visible={showHistory}
      ...
      >
        ...
        >
          ...
          <Column
            field="statut"
            header="Statut"
            body={(r) => <Tag value={r.statut} severity={r.statut === 'Reçue' ? 'success' : r.statut === 'En cours' ? 'warning' : 'info'} />}
            style={{ width: 140 }}
          />
        </DataTable>
      ...
      {/* Styles inline responsive supplémentaires */}
      <style>{`
