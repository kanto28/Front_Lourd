...
export default function StockMonitoringPage() {
    ...
    if (globalFilter) {
      ...
      list = list.filter(
        (r) =>
          ...
          r.fournisseurHabituel.nom.toLowerCase().includes(q)
      );
    }
    if (statutFilter) {
  ...
  const saveSeuilForm = () => {
    ...
    setStocks((s) =>
      s.map((x) =>
        x.id === seuilForm.id
          ? { ...x, seuilMinimum: seuilForm.seuilMinimum, seuilCritique: seuilForm.seuilCritique, stockSecurite: seuilForm.stockSecurite }
          : x
      )
    );
    ...
    confirmDialog({
      ...
      accept: () => {
        toast.current?.show({
          ...
          detail: `Commande simulée pour ${row.nom} (${row.stockSecurite - row.stockActuel} unités).`,
        });
      },
    });
  };
      ...
      case 'aCommander':
        csvContent = [
          ...
          ...filtered
            ...
            .map((r) => [r.nom, r.stockActuel, r.seuilMinimum, r.stockSecurite - r.stockActuel].join(','))
        ].join('\n');
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#eef1f2' }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <i className="pi pi-box" style={{ fontSize: 18 }} />
          <strong>Stock ▸ Monitoring des niveaux</strong>
        </div>
      ...
      <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
        ...
        <main style={{ flex: 1, padding: 16, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
            ...
            <Card
              title="Stock faible"
              subTitle={dashboardData.stockFaible}
              style={{ color: '#f59e0b' }}
              icon="pi pi-exclamation-circle"
            />
            <Card
              title="Stock critique"
              subTitle={dashboardData.stockCritique}
              style={{ color: '#dc2626' }}
              icon="pi pi-exclamation-triangle"
            />
            <Card
              title="Ruptures"
              subTitle={dashboardData.stockRupture}
              style={{ color: '#000' }}
              icon="pi pi-times-circle"
            />
            <Card
              title="Surconsommation"
              subTitle={dashboardData.stockSurconsommation}
              style={{ color: '#3b82f6' }}
        ...
        <div style={{ display: 'grid', gap: 16 }}>
          <span className="p-float-label">
            <InputText
              ...
              onChange={(e) => setSeuilForm({ ...seuilForm, seuilMinimum: parseInt(e.target.value) || 0 })}
              style={{ width: '100%' }}
            />
          ...
          <span className="p-float-label">
            <InputText
              ...
              onChange={(e) => setSeuilForm({ ...seuilForm, seuilCritique: parseInt(e.target.value) || 0 })}
              style={{ width: '100%' }}
            />
          ...
          <span className="p-float-label">
            <InputText
              ...
              onChange={(e) => setSeuilForm({ ...seuilForm, stockSecurite: parseInt(e.target.value) || 0 })}
              style={{ width: '100%' }}
            />
